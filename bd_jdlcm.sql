...
--
-- Temporary view structure for view `datos_matricula`
--

DROP TABLE IF EXISTS `datos_matricula`;
/*!50001 DROP VIEW IF EXISTS `datos_matricula`*/;
SET @saved_cs_client     = @@character_set_client;
/*!50503 SET character_set_client = utf8mb4 */;
/*!50001 CREATE VIEW `datos_matricula` AS SELECT 
 1 AS `#`,
...
INSERT INTO `matricula` VALUES (3,34567,1,3,1,3,'2024-05-20'),(11,11234,1,11,1,11,'2024-05-20'),(12,12345,1,12,2,12,'2024-05-20'),(13,23456,1,13,1,13,'2024-05-20'),(14,34567,1,14,2,14,'2024-05-20'),(15,45678,1,15,1,15,'2024-05-20'),(16,56789,1,1,2,16,'2024-05-20'),(17,67890,1,2,1,17,'2024-05-20'),(18,78901,1,3,2,18,'2024-05-20'),(19,89012,1,4,1,19,'2024-05-20'),(20,90123,1,5,2,20,'2024-05-20'),(21,10123,1,6,1,21,'2024-05-20'),(22,11234,1,7,2,22,'2024-05-20'),(23,12345,1,8,1,23,'2024-05-20'),(24,23456,1,9,2,24,'2024-05-20'),(25,34567,1,10,1,25,'2024-05-20'),(26,45678,1,11,2,26,'2024-05-20'),(27,56789,1,12,1,27,'2024-05-20'),(28,67890,1,13,2,28,'2024-05-20'),(29,78901,1,14,1,29,'2024-05-20'),(30,89012,1,15,2,30,'2024-05-20'),(31,87346,1,2,1,31,'2024-06-22');
...
--
-- Final view structure for view `datos_matricula`
--
...
/*!50001 VIEW `datos_matricula` AS select `mt`.`idMatricula` AS `#`,`mt`.`CodMatricula` AS `Código de matrícula`,`aa`.`Anio_Academico` AS `Año académico`,`al`.`Fecha_Nacimiento` AS `Fecha de nacimiento`,`sx`.`Sexo` AS `Sexo`,group_concat(`nt`.`Nota` order by `nt`.`Nota` ASC separator ', ') AS `Notas`,`al`.`Telefono` AS `Teléfono Alumno`,`tt`.`Telefono` AS `Teléfono Tutor`,`tt`.`Cedula` AS `Cédula Tutor`,`tt`.`Direccion` AS `Dirección Tutor`,trim(`g`.`Grado`) AS `Grado`,trim(`sc`.`NSecc`) AS `Sección`,trim(`t`.`Turno`) AS `Turno`,`pt`.`Parentesco` AS `Parentesco Tutor`,`stx`.`Sexo` AS `Sexo Tutor`,concat(trim(`al`.`PNombre`),' ',trim(`al`.`SNombre`),' ',trim(`al`.`PApellido`),' ',trim(`al`.`SApellido`)) AS `Alumno`,concat(trim(`tt`.`PNombre`),' ',trim(`tt`.`SNombre`),' ',trim(`tt`.`PApellido`),' ',trim(`tt`.`SApellido`)) AS `Tutor` from (((((((((((`matricula` `mt` join `anio_academico` `aa` on((`mt`.`Anio_Academico_idAnio_Academico` = `aa`.`idAnio_Academico`))) join `gradoseccion` `gs` on((`mt`.`GradoSeccion_idGradoSeccion` = `gs`.`idGradoSeccion`))) join `grado` `g` on((`gs`.`Grado_idGrado` = `g`.`idGrado`))) join `seccion` `sc` on((`gs`.`Seccion_idSeccion` = `sc`.`idSeccion`))) join `alumnos` `al` on((`mt`.`Alumnos_idAlumno` = `al`.`idAlumno`))) join `sexo` `sx` on((`al`.`Sexo_idSexo` = `sx`.`idSexo`))) join `tutor` `tt` on((`al`.`Tutor_idTutor` = `tt`.`idTutor`))) join `turno` `t` on((`mt`.`Turno_idTurno` = `t`.`idTurno`))) join `parentesco` `pt` on((`tt`.`Parentesco_idParentesco` = `pt`.`idParentesco`))) join `sexo` `stx` on((`tt`.`Sexo_idSexo` = `stx`.`idSexo`))) left join `nota` `nt` on((`mt`.`idMatricula` = `nt`.`Matricula_idMatricula`))) group by `mt`.`idMatricula` order by (case trim(`g`.`Grado`) when '7mo' then 1 when '8vo' then 2 when '9no' then 3 when '10mo' then 4 when '11mo' then 5 else 6 end),`mt`.`idMatricula` */;
...
-- Dump completed on 2024-06-26 21:46:51
